# Postulación Sercotec — Munnay SpA

Carpeta de trabajo para postular Munnay SpA a un fondo Sercotec.

**Fecha de armado:** 27 de agosto de 2026
**Empresa:** MUNNAY SpA — RUT 77.960.253-2 — Transporte de carga por carretera
**Región de operación:** Antofagasta

## Resumen en tres líneas

1. El fondo que te corresponde es **Crece** (o **Crece Sostenible**, que paga más). Capital Semilla queda descartado porque exige NO tener inicio de actividades, y Munnay ya factura.
2. Las convocatorias Crece 2026 **cerraron en junio y julio**. Lo que hay que hacer ahora es dejar todo listo para la próxima apertura, porque los prerrequisitos toman semanas.
3. **Sercotec no financia camiones.** Las bases excluyen vehículos que requieren inscripción en el Registro Civil. El proyecto tiene que armarse sobre lo que sí es financiable.

## Archivos

| Archivo | Qué contiene |
|---|---|
| `01-diagnostico-munnay.md` | Perfil de la empresa con datos reales, y los problemas que el proyecto debe atacar |
| `02-que-fondo-conviene.md` | Comparación de fondos, cuál aplica y cuál no, con fechas |
| `03-plan-de-inversion.md` | Plan de inversión armado sobre ítems financiables, con montos |
| `04-borrador-formulario.md` | Texto borrador para cada pregunta del formulario y guion del video |
| `05-checklist-y-plazos.md` | Prerrequisitos, documentos y qué hacer esta semana |

## Advertencia sobre las fuentes

No pude acceder a sercotec.cl desde este entorno (bloqueo de red). Todo lo que dice fecha, monto o porcentaje viene de prensa y de fichas de convocatoria indexadas, no de las bases oficiales. **Antes de postular hay que descargar las bases regionales de Antofagasta desde sercotec.cl y verificar cada cifra.** Los puntos que exigen verificación están marcados con `[VERIFICAR]`.
